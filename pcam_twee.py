...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        if float(torch.rand(1, dtype=torch.float64)) < self.probability:
            ...
            sample = rotate(sample, angle, axes=(0, 1), reshape=False, order=1, mode='nearest')
        return sample.copy()
...
class RandomNoise(object):
    ...
    def __call__(self, sample):
        if float(torch.rand(1, dtype=torch.float64)) < self.probability:
            noise = np.random.normal(0, .5, sample.shape)
            sample = sample + noise
        return sample.copy()
...
def Dataloaders(data_dir, batch_size):
    train_transforms = transforms.Compose([RandomRotate(0.5)])
    #train_transforms = transforms.Compose([RandomRotate(0.33), RandomFlip(0.33), RandomNoise(0.33)])
    ...
    train_loader = DataLoader(Dataset(train_x, train_y, train_transforms), batch_size, shuffle=True)
